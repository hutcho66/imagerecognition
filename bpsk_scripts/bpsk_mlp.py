...
"""bpsk_mlp.py: Applies a MLP model to BPSK data."""

import bpsk_generator
...
learning_rate = 0.5
h1_size = 6 #Number of hidden units in hidden layer 1

# Set BPSK signal parameters
training_set_size = 100000
test_set_size = 10000
energy = 4.0
var = (1.0,2.0)  # Set to a tuple for unequal variances
prob_one = 0.5
...
def train():
    ...
    ###### INFERENCE STAGE - build the model
    ...
    # Create a hidden layer with ReLU activation function
    h1 = model_builders.nn_layer(x, 1, h1_size,'hidden',act=tf.nn.relu)
    # Create a layer with identity output to find the pre-activations
    y = model_builders.nn_layer(h1, h1_size, 2, 'output', act=tf.identity)
    
    #Create a single softmax layer
    #y2 = model_builders.nn_layer(x,1,2,'output2',act=tf.identity)

    ###### LOSS STAGE

    # Find the average cross entropy given the current weights
    with tf.name_scope('cross_entropy'):
        diff = tf.nn.softmax_cross_entropy_with_logits(labels=t, logits=y) #applies softmax to y before calculating cross-entropy
        with tf.name_scope('total'):
            cross_entropy = tf.reduce_mean(diff)
    tf.summary.scalar('cross_entropy', cross_entropy) #records cross-entropy
