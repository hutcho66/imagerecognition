...
"""mnist_conv_32.py: Applies a 32 feature convolutional classifier to the MNIST digit dataset."""
...
def train(weight_decay=0.0, keep_prob=1.0):
    ...
    with tf.name_scope('conv_layer1'):
        ...
        h_conv = tf.nn.relu(tf.nn.conv2d(x_reshape, W_1, strides=[1, 1, 1, 1], padding='SAME') + b_1)
    ...
    with tf.name_scope('fully_connected'):
        # Flatten layer
        h_pool_flat = tf.reshape(h_pool, [-1, 6272])
        h_fc = model_builders.nn_layer(h_pool_flat, 6272, 1024, 'fc_1', wd=weight_decay, act=tf.nn.relu)
...
if __name__ == "__main__":
    ...
    model_dir = "../mnist_conv_models/conv_nlayers=1_nfeatures=32"
