...
"""lenet5.py: Applies the LeNet5 network to the MNIST digit dataset."""
...
def train(weight_decay=0.0, keep_prob=1.0):
    ...
    # Layer with 6 features, 5x5 patch, unit stride and zero-padding
    with tf.name_scope('conv_layer1'):
        W_1 = model_builders.weight_variable([5, 5, 1, 6])
        b_1 = model_builders.bias_variable([6])
    ...
    # Layer with 16 features, 5x5 patch, unit stride and no padding
    with tf.name_scope('conv_layer2'):
        W_2 = model_builders.weight_variable([5, 5, 6, 16])
        b_2 = model_builders.bias_variable([16])
    ...
    # Layer with 120 features, 5x5 patch, unit stride and no padding
    with tf.name_scope('conv_layer2'):
        W_3 = model_builders.weight_variable([5, 5, 16, 120])
        b_3 = model_builders.bias_variable([120])
        h_conv3 = tf.nn.relu(tf.nn.conv2d(h_pool2, W_3, strides=[1, 1, 1, 1], padding='VALID') + b_3)

    # Add a fully connected layers of size 84 and dropout
    with tf.name_scope('fully_connected'):
        # Flatten layer
        h_pool3_flat = tf.reshape(h_conv3, [-1, 120])
        h_fc1 = model_builders.nn_layer(h_pool3_flat, 120, 84, 'fc_1', wd=weight_decay, act=tf.nn.relu)
        h_fc1_dropped = tf.nn.dropout(h_fc1, k)

    # Define the intermediate layer. For softmax outputs, we define an pre-activation layer to avoid numerical
    # instability issues.
    y = model_builders.nn_layer(h_fc1_dropped, 84, 10, 'intermediate', wd=weight_decay, act=tf.identity)
...
if __name__ == "__main__":
    ...
    print("Using LeNet5 network (3 convolutional layers, 2 pooling layers and a fully connected layer)")

    # Generate directory structure
    model_dir = "../mnist_conv_models/conv_lenet5_morereg"
